...
**Baseline dependency**: this is a v2 feature. Every task assumes the baseline foundations from `specs/001-phased-product-baseline/tasks.md` exist: `createApp(deps)` and `apps/api/test/harness.ts`, `apps/api/test/ownership.test.ts`, `SecretBox`, `RateLimiter`, `JobRunner` (`apps/api/src/jobs/runner.ts`), `flags` and `audit_log` tables, `users.time_zone`, `PanelState.vue`, `packages/ui` components, `tests/e2e/fixtures/index.ts`, `infra/mocks` (`@desk/mocks`) and `apps/landing/src/pages/privacy.vue`. Do not start Phase 1 until the Phase 6 cut-over in `docs/ROADMAP.md` is done.
...
- [ ] T001 Commit the planning set (`specs/002-mail-calendar-panels/**`, `docs/adr/ADR-0004-mail-and-calendar-panels.md`, edited `CLAUDE.md`, `.specify/memory/constitution.md`) on the current branch so the delivery branch starts from it
...
- [ ] T004 [P] Add the four flag rows `panels.google_calendar`, `panels.google_mail`, `panels.microsoft`, `panels.standards` to the flags seed in `packages/db/src/seed.ts` (off in production, on in local and e2e-ci) and expose them through the existing `GET /flags` in `apps/api/src/routes/flags.ts`
...
- [ ] T009 [P] Extend the routes table in `apps/api/test/ownership.test.ts` with every route in contracts/api.md (`GET /today`, `POST /today/refresh`, `GET /connections`, `PATCH /connections/:id`, `GET /connections/:id/calendars`, `POST /connections/:id/reconnect`, `POST /connections/:id/refresh`, `DELETE /connections/:id`) as user A against user B's ids, expecting `not_found` or empty
...
- [ ] T062 [US3] Complete `apps/web/src/views/ConnectionsView.vue`: per-account card with editable label, colour picker from a fixed palette in `packages/ui/src/tokens.css` (contrast-checked for both themes), pause/resume, calendar checklist from `GET /connections/:id/calendars`, reconnect button (OAuth redirect or password dialog), disconnect with confirm dialog naming what is removed, status badge and last error copy from `apps/web/src/utils/errors.ts`; `?connected=<id>` toast on return from the provider
- [ ] T063 [P] [US3] Handle `purged` and `reconnect_needed` in `apps/web/src/stores/today.ts` and the `PanelState.vue` usage in both panels: loading state instead of stale rows while purged, per-account reconnect prompt that links to `/settings/connections`
...
- [ ] T075 [P] Add `tests/e2e/tests/isolation-panels.spec.ts` (ci): two users each with a mock account; user B never sees user A's chips, events or messages on `/today` or `/settings/connections` (SC-003, browser-level complement to the API ownership matrix)
- [ ] T076 [P] Add a `CHANGELOG.md` entry per slice and update `docs/ROADMAP.md` v2 section with the four slices and their production-flag gates; update the "Current state" section of `CLAUDE.md`
...
- Resolve the [Conflict] items CHK020–CHK023 in `checklists/spec-quality.md` before T027 and T059. Until then these tasks assume: display window today..today+6 with events shown on every day they cover (T026, T033); status `error`, not paused, after twenty failures (T056, T059); revoke is best-effort and audited (T054, T059)
