...
**Baseline dependency**: every task assumes the baseline foundations from `specs/001-phased-product-baseline/tasks.md` exist: `createApp(deps)` and `apps/api/test/harness.ts`, `apps/api/test/ownership.test.ts`, `RateLimiter`, `JobRunner` (`apps/api/src/jobs/runner.ts`), the `flags`, `audit_log`, `fx_rates` and the budget column on `categories` tables, `users.time_zone` and `users.default_currency`, `packages/core/src/month-summary.ts` and `forecast.ts`, the Frankfurter rates client and `FakeRates` (`packages/connectors/src/rates/`), `PanelState.vue`, `packages/ui` components, `tests/e2e/fixtures/index.ts` (`freezeClock`, `axeCheck`), `infra/mocks` (`@desk/mocks`) and `apps/landing/src/pages/privacy.vue`. Slices A, C and D need roadmap Phase 2 (month summary, budgets); Slice B's privacy text needs Phase 4; Slice E needs spec 002 Slice A.
...
- [ ] T003 [P] Add the flag rows `widgets.currency`, `widgets.weather`, `widgets.sunrise`, `widgets.spend_pace`, `widgets.fixed_costs` (seeded `default_on = false`; compose and the e2e-ci job switch them on with `pnpm flags set`) and the value row `widgets.weather_paused_until` (`value` null; column added in T010) to the flags seed in `packages/db/src/seed.ts`, and expose the five boolean flags through `GET /flags` in `apps/api/src/routes/flags.ts`
...
- [ ] T026 [US1] Implement `apps/api/src/jobs/widgets-rates-backfill.ts` (payload `{ base, quote }`; calls `range(today - 31 days, today, base, [quote])` and upserts each returned date into `fx_rates`; one job per new pair, deduped by key `widgets.rates_backfill:<base>:<quote>`) and register it in `apps/api/src/jobs/index.ts`; enqueue it from `create` and `patch` in `apps/api/src/services/widgets.ts` for every code not already present in `fx_rates` for the last 31 days; also enqueue it from the baseline `currency.change` job (`apps/api/src/jobs/currency-change.ts`) for every widget currency against the new default so the changes re-base with history (FR-010)
...
- [ ] T062 [P] Add a `CHANGELOG.md` entry per slice, add the five slices with their flag gates to `docs/ROADMAP.md`, update the "Current state" section of `CLAUDE.md`, and file `needs-rostom` notes in `docs/ROADMAP.md` for the SC-007 check and the SC-005 screen-reader reorder pass, recorded with dates and results when run (three people outside the project add a weather widget unaided and describe what Desk stores)
