...
**Baseline dependency**: starts at the top of roadmap Phase 4 (spec FR-017): the month view, add-expense flow, categories with budgets and settings from `specs/001-phased-product-baseline/tasks.md` exist, as do `packages/ui/src/tokens.css`, the base components from baseline T027 and `tests/e2e/fixtures/index.ts` (`axeCheck`). Baseline T095 (`PanelState.vue`), T105/T107 (`lighthouserc.json`, `lighthouse` step) and T106 (`apps/landing`) are roadmap Phase 4 work that may or may not have landed; T011, T022 and US4 say what to do in each case.
...
- [ ] T004a [P] Wire the unit tests this feature adds into CI: `"test": "vitest run"` plus a `vitest.config.ts` (environment `node`, `include: ['src/**/*.test.ts']`; `apps/web` also `test/**/*.test.ts`) in `packages/ui/package.json` and `apps/web/package.json`, and widen the root `test:unit` script in `package.json` to `pnpm -r --filter @desk/core --filter @desk/ui --filter @desk/web test`; prove it with a placeholder test that fails, then passes, in each package (removed by T005/T006)
...
- [ ] T005 [P] Write failing unit tests in `packages/ui/src/tokens.test.ts`: every name in contracts/tokens.md §Names is defined on `:root` and, for colour tokens, in both dark blocks; every contrast pair in contracts/tokens.md meets its ratio in both themes (parse the CSS values, compute WCAG relative luminance in the test); the `prefers-reduced-motion` block sets the three durations to `0ms` and every `--motion-*` duration is ≤ 300 ms; a drift scan of `apps/web/src` and `packages/ui/src/components` finds no hex/`rgb()`/`hsl()` colour, no `px`/`rem` `font-size`, `margin`, `padding`, `gap` or `transition-duration` literal outside the allow-list (`0`, `1px` borders, `100%`; research R3), reporting file and line; every `.vue` in `packages/ui/src/components` has a sibling `.story.ts` listing at least one variant and one state
...
- [ ] T011 [P] Refresh the surface blocks in `packages/ui/src/components/`: `Sheet.vue` (rename of baseline `Dialog.vue`, update its imports; bottom on phone, side on desktop, `role="dialog"`, focus trap, Esc, focus return), `Toast.vue` (roles, undo, 6 s pause on hover/focus), `PanelFrame.vue` (loading/empty/error with `causeMessages`, self-contained; if baseline T095's `PanelState.vue` already exists, fold it in and delete it, otherwise T095 is done here), `Skeleton.vue`, `EmptyState.vue`, `ErrorState.vue`, `Tile.vue`, `DataTable.vue` (`<caption>`, `aria-sort`, tabular figures) per contracts/blocks.md, with a `.story.ts` beside each; motion via `<Transition>` and the motion tokens only
- [ ] T012 Build the catalogue: `apps/web/src/catalogue/fixtures.ts` (synthetic user, month, categories, budgets and error causes per data-model.md §Catalogue fixtures, plus the spec edge cases: a 60-character category name, a six-digit amount, a day with thirty entries), `apps/web/src/catalogue/CatalogueView.vue` (every story's variants × states in a light and a dark column, anchor per block, theme forced per column with `data-theme` on a wrapper), and mount it at `/__ui` in `apps/web/src/router.ts` only when `import.meta.env.VITE_UI_CATALOGUE === 'true'` as a lazy route; no store or API import anywhere under `catalogue/`
...
**Independent Test**: the owner reads the document and the mock-ups in under fifteen minutes and can say which references were chosen, why, and what changes on the month view; four of five people outside the project shown the before/after mock-ups pick the "after" (SC-002).
...
- [ ] T014 [P] [US1] Extend `tests/e2e/tests/visual.spec.ts` (ci): `/__ui/mockups/month-view`, `/__ui/mockups/add-expense`, `/__ui/mockups/settings` and `/__ui/mockups/month-view-fallback` at 2 × 2, each also with `?state=loading|empty|error` and the month view with `?fixture=edge` (long name truncated with an accessible full value, six-digit amount inside its tile, thirty-entry day keeps table alignment, no horizontal overflow), with axe in both themes; the month-view mock-up's primary tile figure has the largest computed font size on the page (FR-011)
...
- [ ] T017 [US1] Write the proposed values as `apps/web/src/catalogue/proposed-tokens.css` (same token names, `:root` and the two dark blocks) imported only by `CatalogueView.vue` and the mock-ups, so `packages/ui/src/tokens.css` stays the one token set in the product (FR-017) and shipped screens keep the existing system until ADR-0006 is accepted (FR-016); the T006 bundle test proves production carries neither the catalogue nor this file; extend `packages/ui/src/tokens.test.ts` so the contrast pairs are asserted for both files
...
- [ ] T019 [US1] Owner review on the preview URL; record the outcome: set ADR-0006 status to Accepted or Rejected (fallback), update DESIGN.md's state line, and when accepted move the values from `apps/web/src/catalogue/proposed-tokens.css` into `packages/ui/src/tokens.css`; either way delete `proposed-tokens.css` and its imports (T017); when rejected the fallback mock-up becomes the mock-up
...
- [ ] T035 [US4] Apply the tokens to `apps/landing/src/pages/index.vue` (shared `packages/ui/tokens.css`, `Button` for sign-up, the three steps as `Tile`-style cards, `<picture>` switching the light and dark screenshots on `prefers-color-scheme`), with the working name read from `apps/landing/src/content.ts` (baseline T106) until ADR-0002 is decided
...
- [ ] T036 [P] Replace the "Design system" section of `CLAUDE.md` with a pointer to `docs/design/DESIGN.md` plus the four rules that survive regardless (three theme states, status colours never as chart series, hand-written charts, no new runtime dependency); add ADR-0006 to the Key documents list; check every PR of the feature added its `CHANGELOG.md` line; note the slices under Phase 4 in `docs/ROADMAP.md`
- [ ] T037 [P] Record the people-based checks (SC-001 review rounds, SC-002 five outsiders on the mock-ups, SC-003 one screen-reader pass of the five core flows with NVDA or VoiceOver, SC-008 three outsiders on the landing page) with dates and results in `docs/design/DESIGN.md` §Validation, marked `needs-rostom` in `docs/ROADMAP.md` until run
...
- **Setup (Phase 1)**: starts at the top of roadmap Phase 4; T002–T004a in parallel after T001; T004a before any Phase 2 test
...
- `apps/web/src/catalogue/proposed-tokens.css` (T017) exists only between the mock-ups and the ADR-0006 decision and is deleted in T019 either way
