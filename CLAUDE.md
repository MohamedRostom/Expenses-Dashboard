...
- Rostom to-dos, still open: Fly.io account + repo secrets `FLY_API_TOKEN`, `PREVIEW_DATABASE_URL`, `STAGING_DATABASE_URL`; Cloudflare account + `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` (needed once Phase 6 lands); register self-hosted runner `desk-local` and create the `local-secrets` environment with required reviewers; decide the five ADR-0002 items and accept ADR-0001/ADR-0002; run SC-006/SC-009 with real people and record the results.
