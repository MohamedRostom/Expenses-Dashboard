...
- **Branching deviation from the stated convention, flag for Rostom:** CLAUDE.md's convention is one `phase-N/short-description` branch and PR per roadmap phase. In practice this session built Phases 1 through 4 sequentially all on the single branch `phase-1/accounts`, with no per-phase PRs opened or merged. Nothing has been pushed. Before treating any phase as "done" in the PR-and-CI sense, this history needs to be split into phase branches/PRs (or the convention consciously revised) — worth a conversation with Rostom rather than silently reconciling it.
- T123/T124 boundary (see ADRs and `docs/ROADMAP.md`): ADR-0001 and ADR-0002 remain **Proposed**, not accepted — the five ADR-0002 items (product name, email provider, Neon-from-day-one, licence, analytics) are still `_pending_`, and per ADR-0002's own text nothing may treat "Desk" as final until accepted. No renaming has been done. `docs/ROADMAP.md` marks SC-006 (three observed sign-ups) and SC-009 (15-second phone add) as `needs-rostom` — not yet run, since both need real humans an agent cannot supply.
...
- Rostom to-dos, still open: Fly.io account + repo secrets `FLY_API_TOKEN`, `PREVIEW_DATABASE_URL`, `PREVIEW_SESSION_SECRET`, `PREVIEW_SECRET_BOX_KEY`, `STAGING_DATABASE_URL`, `STAGING_SESSION_SECRET`, `STAGING_SECRET_BOX_KEY`, `PRODUCTION_DATABASE_URL`, `PRODUCTION_SESSION_SECRET`, `PRODUCTION_SECRET_BOX_KEY` (C12 fix: the API refuses to start without `SESSION_SECRET`/`SECRET_BOX_KEY`/`APP_ORIGIN`, so preview/staging/production deploys each need their own values, not just `DATABASE_URL`), plus a `production` GitHub Environment with required reviewers for `deploy-fly.yml`'s promote job; Cloudflare account + `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` (needed once Phase 6 lands); register self-hosted runner `desk-local` and create the `local-secrets` environment with required reviewers; decide the five ADR-0002 items and accept ADR-0001/ADR-0002; run SC-006/SC-009 with real people and record the results.
- Next action: reconcile the branching deviation above with Rostom, then decide whether to open per-phase PRs retroactively or restart Phase 5 work on its own branch per convention.
